...
path <- "/Users/sohee/Desktop/02-GEO/Results/송부용/GEO/"
lst <- c("GSE111016_osoh.csv", "GSE167185_osoh.csv", 
         "GSE167185_osyh.csv", "GSE167185_ohyh.csv",
         "merge_osoh.csv")
DEG <- read.csv(paste0(path, 'DEG/', lst[1]))
output <- "/Users/sohee/Desktop/02-GEO/Results/송부용/GEO/Visualization/"

# Z-score 변환
logcpm_corrected <- read.csv(paste0(path, "/Data/GSE111016_cpm_matrix.csv"), 
                             row.names = 1)
...
#                        row.names = samples)
# print(metadata)
metaadata <- read.csv(paste0(path, 'Data/', "GSE111016_metadata.csv"))
metaadata
...
# Heatmap 생성 ------------------
filtered_deg <- DEG[abs(DEG$logFC) > 1 & DEG$FDR < 0.05, ]
