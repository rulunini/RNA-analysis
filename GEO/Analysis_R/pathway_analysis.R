...
# LogFC와 FDR을 변수로 받기
fdr_threshold <- 0.05 
logFC_threshold_up <- 1 
logFC_threshold_down <- -1 

# LogFC ≥ 2 (Up) & FDR < 0.05 유전자 추출
gene_symbols_up <- DEG$Symbol[DEG$logFC > logFC_threshold_up & DEG$FDR < fdr_threshold]
...
gene_symbols_down <- DEG$Symbol[DEG$logFC < logFC_threshold_down & DEG$FDR < fdr_threshold]
